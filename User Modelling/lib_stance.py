...
filename1 = "train.csv"#twitter-2016dev-CE-output.txt_semeval_tweets.txt"

filename2 = "test.csv"#"twitter-2016dev-CE-output.txt_semeval_userinfo.txt"
...
#    user_req_attributes = ['tweet_id', 'user_id']#, 'followers_count', 'statuses_count', 'description', 'friends_count', 'location']
    tweet_req_attributes = ['topic', 'sentiment', 'tweet']
    
    path = cwd + "/data/" + filename1;
    tweet_df = pd.read_csv(path, sep=',')
#    op = 1 - (pd.DataFrame(tweet_df['sentiment']).applymap(np.isreal))
    tweet_df.ix[:, 'sentiment'] = tweet_df['Stance']
...
#    print tweet_df['sentiment']
#    print tweet_df['Stance']
    tweet_df.ix[tweet_df['sentiment']=='pos', 'sentiment'] = 3#1.5
    tweet_df.ix[tweet_df['sentiment']=='other', 'sentiment'] = 0
    tweet_df.ix[tweet_df['sentiment']=='neg', 'sentiment'] = -3#1.5
   
    tweet_df['sentiment'] = pd.to_numeric(tweet_df['sentiment'], errors='coerce')
    tweet_df = tweet_df.dropna(subset=['sentiment'])
#    print tweet_df['sentiment']
    
#    tweet_df.ix[tweet_df['sentim,ent']==1, 'sentiment'] = 3#1.5
...
#    print tweet_df2
#    op = 1 - (pd.DataFrame(tweet_df2['sentiment']).applymap(np.isreal))
    tweet_df2.ix[:, 'sentiment'] = tweet_df2['Opinion Towards']
...
#    tweet_df2['sentiment'] = pd.to_numeric(tweet_df2['sentiment'])
    ...
    tweet_df2.ix[tweet_df2['sentiment']== 'other', 'sentiment'] = 0#.ix[tweet_df2['sentiment']=='neutral', 'sentiment'] = 0
    tweet_df2.ix[tweet_df2['sentiment']=='pos', 'sentiment'] = 3#1.5
    tweet_df2.ix[tweet_df2['sentiment']=='neg', 'sentiment'] = -3#1.5
    tweet_df2['sentiment'] = pd.to_numeric(tweet_df2['sentiment'], errors='coerce')
    tweet_df2 = tweet_df2.dropna(subset=['sentiment'])
...
#    #data = data[data["topic"].isin(all_topics)]
    ...
    print Counter(data_test['sentiment'])
    print "indexes reset"
    data = data.reset_index()
    data_test = data_test.reset_index()
#    data0 = data0.reset_index()
#    data3 = data3.reset_index()
    return [data, data_test]#[req_attributes]
